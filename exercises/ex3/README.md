...
We will then vectorize a raster file which contains information about population density and upload the data using [GDAL](http.gdal.org).
...
Let's add an additional `GROUP BY` attribute to get the distribution of POI types (`tags.amenity`).
...
This query assigns a cluster ID (`CID`) and cell (`CCELL`) to each POI.
...
Now let's intersect the `V_TRIP_LINES` view which we created in the first exercise.
...
We can overlay the `V_ROUTE_LINES` to add context.
